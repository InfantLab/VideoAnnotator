...
def test_scene_pipeline(video_path: Path, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if results.get('scenes'):
        ...
        for i, scene in enumerate(results['scenes'][:5]):  # Show first 5 scenes
            logger.info(f"    Scene {i+1}: {scene.get('start_time', 0):.2f}s - {scene.get('end_time', 0):.2f}s")
...
def test_audio_pipeline(audio_path: Path, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if results.get('speech_transcription'):
        logger.info(f"  - Speech transcription: {results['speech_transcription']['text'][:100]}...")
    
    if results.get('speaker_diarization'):
        logger.info(f"  - Number of speakers: {results['speaker_diarization']['num_speakers']}")
    
    return results


def main():
    parser = argparse.ArgumentParser(description='Test individual VideoAnnotator pipelines')
    parser.add_argument('--pipeline', type=str, required=True, 
                       choices=['scene', 'person', 'face', 'audio'],
                       help='Pipeline to test')
    parser.add_argument('--video_path', type=str, help='Path to input video file')
    parser.add_argument('--audio_path', type=str, help='Path to input audio file (for audio pipeline)')
