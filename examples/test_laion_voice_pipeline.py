...
def test_laion_voice_pipeline(video_path: str, output_dir: str, config: dict = None):
    ...
    if results:
        ...
        for i, segment in enumerate(results[:3]):  # Show first 3 segments
            ...
            if "emotions" in segment and segment["emotions"]:
                top_emotions = ", ".join([
                    f"{emotion}({data['score']:.2f})" 
                    for emotion, data in list(segment["emotions"].items())[:3]  # Top 3 emotions
                ])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="./laion_voice_test_results", help="Output directory")
    parser.add_argument("--log_level", type=str, default="INFO", help="Logging level")
    parser.add_argument("--model_size", type=str, default="small", choices=["small", "large"], help="Model size")
