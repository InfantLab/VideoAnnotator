...
def process_single_video(video_path: Path, output_dir: Path, config: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for pipeline_name, pipeline in pipelines.items():
            try:
                ...
                logger.error(f"Error in {pipeline_name} pipeline for {video_path.name}: {e}")
...
def process_videos_batch(video_files: List[Path], output_dir: Path, config: Dict[str, Any], 
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all jobs
        future_to_video = {
            executor.submit(process_single_video, video_path, output_dir, config): video_path
...
def main():
    parser = argparse.ArgumentParser(description='Batch process videos through VideoAnnotator pipeline')
    ...
    try:
        ...
        batch_results = process_videos_batch(video_files, output_dir, config, args.max_workers)
