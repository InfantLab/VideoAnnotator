...
class TestConvenienceFunctions:
    ...
    def test_run_size_based_analysis(self):
        ...
        labels = run_size_based_analysis(annotations, height_threshold=0.5, confidence=0.9)
        
        assert len(labels) == 2
        assert all(info["confidence"] == 0.9 for info in labels.values())
        assert all(info["metadata"]["threshold_used"] == 0.5 for info in labels.values())
...
class TestEdgeCases:
    ...
    def test_all_same_height(self):
        ...
        assert all(info["metadata"]["normalized_height"] == 1.0 for info in labels.values())
