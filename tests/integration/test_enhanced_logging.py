...
from src.utils.model_loader import log_model_download, setup_download_logging, log_first_run_info
...
class TestEnhancedLoggingIntegration:
    ...
    def test_cli_logging_setup(self, caplog):
        ...
        log_messages = [record.message for record in caplog.records]
        # May or may not show first-run info depending on models directory
    ...
    def test_whisper_pipeline_logging_integration(self, caplog):
        ...
        def mock_whisper_load_model(model_size, device="cpu", download_root=None, in_memory=True):
    ...
    def test_multiple_pipeline_loading_sequence(self, caplog):
        ...
        # Should see success messages for all
    ...
    def test_error_handling_in_pipeline_context(self, caplog):
        ...
        log_messages = [record.message for record in caplog.records]
        error_messages = [msg for msg in log_messages if "❌" in msg]
...
class TestFirstRunExperience:
    ...
    def test_complete_first_run_flow(self, mock_exists, caplog, tmp_path):
        ...
        assert any("🎉 Welcome" in msg for msg in log_messages)
        ...
        success_count = len([msg for msg in log_messages if "✅" in msg])
    ...
    def test_subsequent_run_experience(self, caplog):
        ...
        with caplog.at_level(logging.INFO):
            ...
            model = log_model_download(
                "Cached Model",
                "/existing/path/model.pt",
                mock_existing_loader
            )
