...
class TestProgressTrackerRealistic:
    ...
    def test_realistic_batch_progress(self):
        ...
        jobs = [
            # Completed jobs
            self.create_test_job("vid001", JobStatus.COMPLETED,
                               started_at=now - timedelta(minutes=60),
                               completed_at=now - timedelta(minutes=45)),
            self.create_test_job("vid002", JobStatus.COMPLETED,
                               started_at=now - timedelta(minutes=45),
                               completed_at=now - timedelta(minutes=30)),
            
            # Failed jobs
            self.create_test_job("vid003", JobStatus.FAILED,
                               started_at=now - timedelta(minutes=30),
                               error_message="Corrupted video file",
                               retry_count=3),
            
            # Currently running
            self.create_test_job("vid004", JobStatus.RUNNING,
                               started_at=now - timedelta(minutes=10)),
            self.create_test_job("vid005", JobStatus.RUNNING,
    ...
    def test_long_running_batch(self):
        ...
        for i in range(50):
            ...
            jobs.append(self.create_test_job(
                f"completed_{i:03d}", JobStatus.COMPLETED,
        ...
        for i in range(5):
            ...
            jobs.append(self.create_test_job(
                f"failed_{i:03d}", JobStatus.FAILED,
                started_at=job_start,
        ...
        for i in range(10):
            ...
            jobs.append(self.create_test_job(
                f"running_{i:03d}", JobStatus.RUNNING,
                started_at=job_start
            ))
