...
async def test_background_job_processing():
    ...
    try:
        ...
        response = requests.get(
            "http://localhost:8899/api/v1/debug/background-jobs", timeout=5
        )
        assert response.status_code == 200
        ...
        if pending_count > 0:
            ...
            for check_round in range(3):
                ...
                _finished_count = cursor.fetchone()[0]
                conn.close()
                ...
                pytest.skip(
                    "No job processing activity detected - pipelines may still be initializing"
                )
...
async def test_background_processing_endpoints():
    ...
    try:
        response = requests.get(
            "http://localhost:18011/api/v1/debug/background-jobs", timeout=2
        )
