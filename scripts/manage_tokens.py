...
from src.auth import get_token_manager, initialize_token_manager, TokenType
...
def create_api_key(args):
    ...
    username = args.username or input(f"Enter username (default: {email.split('@')[0]}): ") or email.split('@')[0]
    ...
    if args.scopes:
        ...
        scope_input = input("Enter scopes (comma-separated, default: read,write): ") or "read,write"
    ...
    if expires_in_days is None and not args.no_expiry:
        try:
            days_input = input("Expires in days (default: 365, 0 for no expiry): ") or "365"
    ...
    try:
        ...
        for i, token in enumerate(tokens, 1):
            ...
            if token.expires_at:
                ...
                print(f"   Expires:   {token.expires_at.strftime('%Y-%m-%d %H:%M:%S')} ({status})")
            else:
                print("   Expires:   Never")
            
            if token.last_used_at:
                print(f"   Last Used: {token.last_used_at.strftime('%Y-%m-%d %H:%M:%S')}")
            else:
...
def revoke_token(args):
    ...
    try:
        # Confirm revocation
        if not args.force:
            confirm = input(f"Are you sure you want to revoke token {args.token[:20]}...? [y/N]: ")
            if confirm.lower() != 'y':
    ...
    try:
        ...
        if token_info:
            ...
            if token_info.expires_at:
                ...
                status = "EXPIRED" if days_left < 0 else f"{days_left} days left"
                print(f"Expires:   {token_info.expires_at.strftime('%Y-%m-%d %H:%M:%S')} ({status})")
            else:
                print("Expires:   Never")
...
Examples:
  ...
  python scripts/manage_tokens.py stats
    ...
    create_parser.add_argument('--scopes', help='Comma-separated scopes (default: read,write)')
    create_parser.add_argument('--expires-days', type=int, help='Token expires in N days')
    create_parser.add_argument('--no-expiry', action='store_true', help='Create token without expiration')
    ...
    list_parser.add_argument('--show-metadata', action='store_true', help='Show token metadata')
    ...
    revoke_parser.add_argument('--force', '-f', action='store_true', help='Skip confirmation')
    
    # Validate token command
    validate_parser = subparsers.add_parser('validate', help='Validate an API token')
    validate_parser.add_argument('--token', '-t', required=True, help='Token to validate')
