...
def demo_cross_pipeline_person_identity():
    ...
    person_ids_from_faces = set(ann.get('person_id') for ann in linked_faces if ann.get('person_id'))
    
    consistent_ids = person_ids_from_tracking.intersection(person_ids_from_faces)
    
    print(f"✅ Cross-pipeline consistency:")
    ...
    print("=" * 35)
    print(f"✅ Person tracking: {len(person_results)} detections, {len(person_ids)} unique persons")
    print(f"✅ Face analysis: {len(face_annotations)} detections, {len(linked_faces)} linked to persons")
    ...
    if len(consistent_ids) > 0:
        ...
        print("Face-to-person linking is working across pipelines with consistent person identities.")
        return True
    else:
