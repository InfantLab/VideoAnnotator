...
def load_existing_person_tracks(video_path: str, results_dir: str) -> Optional[PersonIdentityManager]:
...
def display_person_analysis(person_stats: Dict[str, Dict]):
    ...
    for i, (person_id, stats) in enumerate(sorted_persons, 1):
        ...
        if stats['current_label']:
            print(f"   Current label: {stats['current_label']} "
                  f"(confidence: {stats['label_confidence']:.2f})")
        else:
            print("   Current label: <not labeled>")


def interactive_labeling_session(identity_manager: PersonIdentityManager, 
    ...
    while True:
        ...
        unlabeled = [pid for pid, stats in person_stats.items() 
                    if not stats.get('current_label')]
        ...
        command = input("\nEnter command (person_id label, 'list', 'help', 'quit'): ").strip()
        ...
        if command.lower() in ['quit', 'q', 'exit']:
            ...
            # Apply label
            identity_manager.set_person_label(person_id, normalized_label, 1.0, "manual")
...
def apply_bulk_labeling(identity_manager: PersonIdentityManager, 
                       person_stats: Dict[str, Dict], 
                       bulk_rules: str):
    ...
    for rule in rules:
        ...
        if criterion == 'largest':
            ...
            largest_person = max(person_stats.items(), 
                               key=lambda x: x[1]['avg_bbox_area'])
            person_id = largest_person[0]
            identity_manager.set_person_label(person_id, normalized_label, 0.8, "bulk_largest")
            person_stats[person_id]['current_label'] = normalized_label
            ...
            smallest_person = min(person_stats.items(), 
                                key=lambda x: x[1]['avg_bbox_area'])
            person_id = smallest_person[0]
            identity_manager.set_person_label(person_id, normalized_label, 0.8, "bulk_smallest")
            person_stats[person_id]['current_label'] = normalized_label
            person_stats[person_id]['label_confidence'] = 0.8
            ...
            most_detected = max(person_stats.items(), 
                              key=lambda x: x[1]['detections'])
            person_id = most_detected[0]
            identity_manager.set_person_label(person_id, normalized_label, 0.8, "bulk_most_detections")
            person_stats[person_id]['current_label'] = normalized_label
            person_stats[person_id]['label_confidence'] = 0.8
            print(f"✓ Labeled most detected person {person_id} as '{normalized_label}'")


def save_updated_annotations(annotations: List[Dict], 
...
def main():
    parser = argparse.ArgumentParser(description="Person labeling tool for VideoAnnotator")
    parser.add_argument("video", help="Path to video file")
    parser.add_argument("--results-dir", "-r", default="./results", 
                       help="Directory containing tracking results")
    parser.add_argument("--output-dir", "-o", default="./labeled_results",
                       help="Output directory for labeled results")
    parser.add_argument("--bulk-label", "-b", help="Bulk labeling rules (e.g., 'largest=parent,smallest=infant')")
    ...
    if identity_manager is None:
        ...
        identity_manager = PersonIdentityManager.from_person_tracks(annotations, video_id)
    ...
    save_updated_annotations(annotations, identity_manager, str(updated_annotations_file))
