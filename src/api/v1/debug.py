...
def log_request(method: str, path: str, status_code: int, response_time_ms: int, user_id: str = None):
    ...
    try:
        ...
        try:
            ...
            if torch.cuda.is_available():
                gpu_info = {
                    ...
                    "memory_allocated_gb": round(torch.cuda.memory_allocated(0) / 1024**3, 2) if torch.cuda.device_count() > 0 else 0,
                    "memory_reserved_gb": round(torch.cuda.memory_reserved(0) / 1024**3, 2) if torch.cuda.device_count() > 0 else 0
                }
            else:
        ...
        return {
            ...
            "database": {
                ...
                "path": db_info.get("connection_info", {}).get("database_path", "unknown"),
                ...
                "size_mb": db_info.get("connection_info", {}).get("database_size_mb", 0)
            },
            ...
            "request_stats": {
                ...
                "requests_last_hour": len([r for r in _request_log if 
                    datetime.fromisoformat(r["timestamp"]) > datetime.now() - timedelta(hours=1)]),
                "average_response_time_ms": sum(r["response_time_ms"] for r in _request_log[-10:]) / min(len(_request_log), 10) if _request_log else 0
            }
        }
        ...
        return {
            "token": {
                ...
                "rate_limit": {
                    ...
                    "reset_at": datetime.now().replace(second=0, microsecond=0) + timedelta(minutes=1)
                }
            },
            "session": {
                ...
                "total_requests": len([r for r in _request_log if r.get("user_id") == current_user.get("id")])
            }
        }
                ...
                "components": [
                    {
                        ...
                        "parameters": {
                            ...
                            "iou_threshold": {"type": "float", "default": 0.7, "min": 0.0, "max": 1.0},
                            "max_detections": {"type": "int", "default": 100, "min": 1, "max": 1000}
                        }
                    },
                    {
                        "name": "bytetrack",
                        ...
                        "model_loaded": True,
                        "parameters": {
                            "track_buffer": {"type": "int", "default": 30, "min": 1, "max": 100},
                            "match_threshold": {"type": "float", "default": 0.8, "min": 0.0, "max": 1.0}
                        }
                    }
                ]
            },
            {
                ...
                "components": [
                    ...
                    },
                    {
                        ...
                        "enabled": True,
                        "model_loaded": True,
                        "supported_analyses": ["age", "gender", "emotion", "attractiveness", "race"],
                        "parameters": {
                            "enable_age_estimation": {"type": "bool", "default": False},
                            "enable_emotion_recognition": {"type": "bool", "default": True},
                            "enable_gender_detection": {"type": "bool", "default": False},
                            "emotion_categories": {
                                "type": "list", 
                                "default": ["happy", "sad", "angry", "surprised", "neutral", "fear", "disgust"],
                                "available": ["happy", "sad", "angry", "surprised", "neutral", "fear", "disgust", "contempt"]
                            }
                        }
                    }
                ]
                ...
                "components": [
                    {
                        ...
                        "model_size": "base",
                        "parameters": {
                            "model_size": {"type": "select", "default": "base", "options": ["tiny", "base", "small", "medium", "large"]},
                            "language": {"type": "select", "default": "auto", "options": ["auto", "en", "es", "fr", "de", "it"]},
                    ...
                    },
                    {
                        ...
                        "model_loaded": True,
                        "parameters": {
                            "min_speakers": {"type": "int", "default": 1, "min": 1, "max": 10},
                            "max_speakers": {"type": "int", "default": 5, "min": 1, "max": 10}
                        }
                    },
                    {
                        "name": "laion_voice",
                        ...
                        "parameters": {
                            ...
                            "emotion_categories": {
                                ...
                                "available": ["neutral", "happy", "sad", "angry", "surprised", "fear", "disgust"]
                            }
                        }
                    }
                ]
                ...
                "components": [
                    {
                        ...
                        "model_loaded": True,
                        "parameters": {
                            "threshold": {"type": "float", "default": 30.0, "min": 1.0, "max": 100.0},
                            "min_scene_len": {"type": "float", "default": 1.0, "min": 0.1, "max": 10.0}
                        }
                    },
                    {
                        ...
                        "model_loaded": True,
                        "parameters": {
                            "confidence_threshold": {"type": "float", "default": 0.3, "min": 0.0, "max": 1.0},
                            "scene_categories": {
                                "type": "list",
                                "default": ["indoor", "outdoor", "office", "home", "restaurant", "street"],
                                "available": ["indoor", "outdoor", "office", "home", "restaurant", "street", "park", "car", "classroom", "kitchen"]
                            }
                        }
                    }
                ]
...
async def get_job_debug_info(
    ...
    try:
        ...
        debug_info = {
            "job": {
                ...
                "debug_info": {
                    ...
                    "processing_start_time": job.started_at.isoformat() if job.started_at else None,
                ...
                "pipeline_logs": [
                    {"timestamp": "10:00:15", "level": "INFO", "message": "Starting job processing"},
                    {"timestamp": "10:01:30", "level": "DEBUG", "message": "Processing video analysis"},
                    {"timestamp": "10:02:45", "level": "INFO", "message": "Pipeline processing in progress"}
                ],
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get job debug info: {str(e)}")


@router.get("/request-log")
async def get_request_log(
    limit: int = Query(50, description="Maximum number of requests to return", ge=1, le=100),
    current_user: dict = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get request log: {str(e)}")


@router.get("/mock-events")
async def mock_sse_events(
    token: Optional[str] = Query(None, description="API token for authentication"),
    job_id: Optional[str] = Query(None, description="Job ID to monitor")
):
    """
...
def _get_pipeline_status() -> Dict[str, Any]:
    ...
    return {
        "initialized": ["person_tracking", "face_analysis", "scene_detection", "audio_processing"],
        "available": ["person_tracking", "face_analysis", "scene_detection", "audio_processing"], 
        "loading_errors": [],
        "total_available": 4,
