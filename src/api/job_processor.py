    ...
    def _process_pipeline(self, job: BatchJob, pipeline_name: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                if pipeline_name in ['audio_processing', 'audio'] and pipeline_class.__name__ == 'AudioPipelineModular':
            ...
            logger.error(f"Pipeline {pipeline_name} failed for job {job.job_id}: {e}", exc_info=True)
            job.error_message = f"Pipeline {pipeline_name} failed: {str(e)}"
            return False