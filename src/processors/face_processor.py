...
def create_faces_dataframe():
    ...
    """
    cols = [
        'frame', 'face_idx', 'face_x', 'face_y', 'face_w', 'face_h',
...
def extract_faces_from_image(frame, backend='opencv', features=['emotion','age','gender'], precision=6, debug=False):
    """
    ...
    try:
        ...
        for face_idx, face in enumerate(faces):
            ...
            if 'emotion' in features:
                ...
                face_data['dominant_emotion'] = max(emotions, key=emotions.get) if emotions else None
...
def extract_faces_from_video(video_path, output_file=None, backend='opencv', skip_frames=0, features=("emotion", "age", "gender"), precision=5):
    """
    ...
    while True:
        ...
        try:
            faces_in_frame = extract_faces_from_image(frame, backend=backend, features=features, precision=precision)
    ...
    if all_faces_data:
        ...
        columns = ['frame', 'timestamp'] + [col for col in all_faces_data[0].keys() if col not in ['frame', 'timestamp']]
...
def match_faces_to_poses(faces_df, poses_df):
    ...
    if len(merged_df) > len(poses_df):
        ...
        merged_df = merged_df.groupby('frame').apply(select_best_face_for_pose).reset_index(drop=True)
...
def select_best_face_for_pose(group):
    ...
    if nose_x is not None and nose_y is not None:
        ...
        group['distance_to_nose'] = np.sqrt((group['face_center_x'] - nose_x)**2 + 
                                          (group['face_center_y'] - nose_y)**2)
...
def process_video_faces(video_path, output_dir, video_metadata=None, poses_df=None, 
                      skip_frames=0, backend='opencv', force_process=False, precision=6, debug=False):
    """
...
def get_facial_stats(faces_df, debug=False):
    ...
    stats["avg_faces_per_frame"] = stats["total_faces"] / stats["unique_frames"] if stats["unique_frames"] > 0 else 0
    ...
    if "face_id" in faces_df.columns:
        ...
        for face_id, group in face_groups:
            ...
            if emotion_cols:
                ...
                face_stats["dominant_emotions"] = emotion_only.idxmax(axis=1).str.replace("emotion_", "").value_counts().to_dict()
            ...
            if debug:
                ...
                if "dominant_emotions" in face_stats:
                    ...
                    for emotion, count in face_stats["dominant_emotions"].items():
                        print(f"    {emotion}: {count} frames ({count/face_stats['count']*100:.1f}%)")
    ...
    if emotion_cols:
        ...
        stats["dominant_emotions"] = emotion_only.idxmax(axis=1).str.replace("emotion_", "").value_counts().to_dict()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip", "-s", type=int, default=0, help="Frames to skip between processing")
    parser.add_argument("--force", "-f", action="store_true", help="Force reprocessing of existing files")
    parser.add_argument("--precision", "-p", type=int, default=8, help="Decimal precision for emotion values")
