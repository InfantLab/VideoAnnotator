...
class SecureTokenManager:
        ...
        try:
            ...
            for token_id, token_info in self._token_cache.items():
                tokens_data[token_id] = {
                    ...
                    'last_used_at': token_info.last_used_at.isoformat() if token_info.last_used_at else None,
    ...
    def generate_api_key(self,
                        ...
                        metadata: Dict[str, Any] = None) -> Tuple[str, TokenInfo]:
        """
    ...
    def generate_session_token(self,
                              ...
                              email: str, 
                              scopes: List[str] = None,
                              expires_in_hours: int = 24) -> Tuple[str, TokenInfo]:
        """
        ...
        logger.info(f"Generated session token for user {username} (expires in {expires_in_hours}h)")
    ...
    def _validate_jwt_token(self, token: str) -> Optional[TokenInfo]:
    ...
    def list_user_tokens(self, user_id: str) -> List[TokenInfo]:
        ...
        return [
            token_info for token_info in self._token_cache.values()
...
def initialize_token_manager(secret_key: Optional[str] = None, 
                           tokens_dir: str = "tokens") -> SecureTokenManager:
    """Initialize the global token manager."""
    global _token_manager
    _token_manager = SecureTokenManager(
        secret_key=secret_key,
        tokens_file=f"{tokens_dir}/tokens.json"
    )
    return _token_manager