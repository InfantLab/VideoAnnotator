...
def process_video(model, video_path, data_out, metadata_row, force_process=False):
    ...
    try:
        ...
        if not force_process and not pd.isnull(row["Keypoints.file"].values[0]) and os.path.exists(row["Keypoints.file"].values[0]):
...
def normalize_and_save_keypoints(video_row, data_out):
    ...
    if 'Keypoints.normed' in processedvideos.columns and pd.api.types.is_float_dtype(processedvideos['Keypoints.normed']):
...
def process_all_videos(videos_in, data_out, metadata_file=None, force_metadata=False, force_process=False):
    """
...
def understand_videos(videos_in, data_out):
    ...
    for index, row in processedvideos.iterrows():
        ...
        processedvideos.at[index, "Understanding.when"] = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())
