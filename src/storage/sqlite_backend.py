...
from .models import (
    Job, PipelineResult, Annotation, BatchReport as BatchReportModel, 
...
class SQLiteStorageBackend(StorageBackend):
    ...
    def __init__(self, database_path: Optional[Path] = None, echo: bool = False):
        ...
        if not database_exists:
            self.logger.info(f"[DATABASE] Creating new VideoAnnotator database: {self.database_path}")
        else:
            self.logger.info(f"[DATABASE] Using existing database: {self.database_path}")
        ...
        try:
            ...
            with self.SessionLocal() as session:
                ...
                if not schema_version:
                    ...
                    session.commit()
                    self.logger.info(f"[DATABASE] Initialized schema version {CURRENT_SCHEMA_VERSION}")
                
                elif schema_version.version < CURRENT_SCHEMA_VERSION:
                    # Future: Handle schema migrations
                    self.logger.warning(f"[DATABASE] Schema version {schema_version.version} < {CURRENT_SCHEMA_VERSION} - migrations needed")
    ...
    def _db_job_to_batch_job(self, db_job: Job) -> "BatchJob":
        ...
        for result in db_job.pipeline_results:
            pipeline_result = BatchPipelineResult(
                ...
                processing_time=result.processing_time / 1000.0 if result.processing_time else None,  # Convert ms to seconds
    ...
    def save_job_metadata(self, job: "BatchJob") -> None:
        ...
        try:
                ...
                if existing:
                    ...
                    for name, result in job.pipeline_results.items():
                        db_result = PipelineResult(
                            ...
                            start_time=result.start_time,
                            end_time=result.end_time,
                            processing_time=int(result.processing_time * 1000) if result.processing_time else None,  # Convert to ms
                            annotation_count=result.annotation_count,
                    ...
                    for name, result in job.pipeline_results.items():
                        db_result = PipelineResult(
                            ...
                            start_time=result.start_time,
                            end_time=result.end_time,
                            processing_time=int(result.processing_time * 1000) if result.processing_time else None,
                            annotation_count=result.annotation_count,
                            output_file=str(result.output_file) if result.output_file else None,
                            error_message=result.error_message
                        )
            ...
            self.logger.error(f"[ERROR] Failed to save job metadata for {job.job_id}: {e}")
            raise
    ...
    def save_annotations(self, job_id: str, pipeline: str, annotations: List[Dict[str, Any]]) -> str:
        ...
        try:
            with self.SessionLocal() as session:
                ...
                self.logger.debug(f"[DATABASE] Saved {len(annotations)} annotations for {job_id}/{pipeline}")
                return f"database://annotations/{job_id}/{pipeline}"
        
        except SQLAlchemyError as e:
            self.logger.error(f"[ERROR] Failed to save annotations for {job_id}/{pipeline}: {e}")
            raise
    ...
    def load_annotations(self, job_id: str, pipeline: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"[ERROR] Failed to load annotations for {job_id}/{pipeline}: {e}")
            raise
    ...
    def annotation_exists(self, job_id: str, pipeline: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"[ERROR] Failed to check annotations for {job_id}/{pipeline}: {e}")
            return False
    ...
    def delete_job(self, job_id: str) -> None:
        ...
        try:
            with self.SessionLocal() as session:
                ...
                if deleted_count == 0:
                    self.logger.warning(f"[WARNING] Job {job_id} not found for deletion")
                else:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            with self.SessionLocal() as session:
                ...
                cancelled_jobs = session.query(Job).filter_by(status="cancelled").count()
        ...
        try:
            with self.SessionLocal() as session:
                ...
                db_report = BatchReportModel(
                    ...
                    total_processing_time=int(report.total_processing_time * 1000) if report.total_processing_time else 0,
            ...
            self.logger.error(f"[ERROR] Failed to save batch report {report.batch_id}: {e}")
            raise
    ...
    def load_report(self, batch_id: str) -> "BatchReport":
        ...
        try:
            with self.SessionLocal() as session:
                db_report = session.query(BatchReportModel).filter_by(id=batch_id).first()
    ...
    def list_reports(self) -> List[str]:
        ...
        try:
            with self.SessionLocal() as session:
                return [row[0] for row in session.query(BatchReportModel.id).order_by(BatchReportModel.start_time.desc()).all()]
    ...
    def cleanup_old_files(self, max_age_days: int) -> tuple[int, int]:
        ...
        try:
            ...
            with self.SessionLocal() as session:
                ...
                # Delete old reports
                old_reports = session.query(BatchReportModel).filter(
                    BatchReportModel.end_time < cutoff_date
                ).all()
                ...
                self.logger.info(f"[DATABASE] Cleaned up {deleted_jobs} old jobs and {deleted_reports} old reports")
