...
class PersonTrackingPipelineStandards(BasePipeline):
                    ...
                    if int(box.cls[0]) == 0:  # Person class
                        ...
                        if keypoints is not None and i < len(keypoints):
                            ...
                            for kp in kp_data:
                                ...
                                visibility = 2 if conf > self.config["min_keypoint_confidence"] else 0
                                if visibility > 0:
                                    visible_keypoints += 1
    ...
    def _save_coco_annotations(
        ...
        if validation_result.is_valid:
            ...
            self.logger.warning(f"Person tracking COCO validation warnings: {', '.join(validation_result.warnings)}")
