...
class LAIONVoicePipeline(WhisperBasePipeline):
    ...
    def initialize(self) -> None:
        ...
        self.logger.info(f"LAIONVoicePipeline initialized successfully")
    ...
    def process(
        ...
        try:
            ...
            for i, segment in enumerate(segments):
                ...
                self.logger.debug(f"Processing segment {i+1}/{len(segments)}: {start_time:.2f}s - {end_time:.2f}s")
                ...
                if embedding is None:
                    self.logger.warning(f"Skipping segment {i+1} due to embedding failure")
                    continue
                ...
                transcription = None
                if include_transcription or self.config.get("include_transcription", False):
                    try:
                ...
                if self.config.get("include_raw_scores", False) and "raw_scores" in emotion_results:
                ...
                if self.config.get("include_transcription", False):
                    ...
                    try:
                        with open(vtt_path, 'w') as f:
                            f.write("WEBVTT\n")
                            f.write(f"NOTE Generated by LAION Voice Pipeline\n\n")
    ...
    def _load_classifiers(self) -> None:
        ...
        # Repository mapping
        repo_id = "laion/Empathic-Insight-Voice-Small" if model_size == "small" else "laion/Empathic-Insight-Voice-Large"
        ...
        if self.device.type == "cuda":
            try:
                cuda_capability = torch.cuda.get_device_capability(self.device.index if hasattr(self.device, 'index') else 0)
                cuda_capability_major = cuda_capability[0] if isinstance(cuda_capability, tuple) else cuda_capability
                if cuda_capability_major >= 7:
                    self.logger.info(f"CUDA capability {cuda_capability_major}.{cuda_capability[1] if isinstance(cuda_capability, tuple) else 'x'} detected - torch.compile optimization available")
                else:
                    self.logger.info(f"CUDA capability {cuda_capability_major}.{cuda_capability[1] if isinstance(cuda_capability, tuple) else 'x'} detected - using standard models (torch.compile requires >=7.0)")
            except Exception as e:
        ...
        for label, filename in EMOTION_LABELS.items():
            ...
            if local_path.exists():
                try:
                    ...
                    needs_stripping = any(k.startswith("_orig_mod.") for k in state_dict.keys())
                    if needs_stripping:
                        stripped_state_dict = {
                    ...
                    if self.device.type == "cuda" and hasattr(torch, "compile") and torch.__version__ >= "2.0.0":
                        ...
                        if cuda_capability_major >= 7:
                            try:
                                ...
                                self.logger.debug(f"Using compiled model for {label} (CUDA capability {cuda_capability_major}.{cuda_capability[1] if isinstance(cuda_capability, tuple) else 'x'})")
                            except Exception as e_compile:
                                self.logger.warning(f"torch.compile failed for {label}: {e_compile}")
                        else:
                            self.logger.debug(f"Skipping torch.compile for {label} (CUDA capability {cuda_capability_major}.{cuda_capability[1] if isinstance(cuda_capability, tuple) else 'x'} < 7.0, triton not supported)")
        ...
        self.logger.info(f"Loaded {loaded_count}/{len(EMOTION_LABELS)} emotion classifiers")
    ...
    def _segment_audio(
        ...
        # Voice Activity Detection (VAD) - simplified approach
            ...
            self.logger.warning(f"Unsupported segmentation mode '{segmentation_mode}' or missing required data. Falling back to fixed interval.")
            return self._segment_audio(audio, sample_rate, pps, start_time, end_time)
    ...
    @torch.no_grad()
    def _get_whisper_embedding(self, audio_segment: np.ndarray) -> Optional[torch.Tensor]:
        """
        ...
        try:
            ...
            embedding = self.get_whisper_embedding(
                audio_segment, 
                pad_or_trim=True, 
                target_seq_len=WHISPER_SEQ_LEN
            )
            return embedding
    ...
    def _predict_emotions(self, embedding: torch.Tensor) -> Dict[str, Any]:
            ...
            try:
                with torch.no_grad():
                    ...
                    if working_embedding.device != classifier_param.device:
                        working_embedding = working_embedding.to(classifier_param.device)
                    
                    # Ensure embedding dtype matches classifier dtype
                    if working_embedding.dtype != classifier_param.dtype:
                        working_embedding = working_embedding.to(dtype=classifier_param.dtype)
                ...
                if "triton" in error_msg or "cuda capability" in error_msg or "compile" in error_msg:
                    self.logger.warning(f"Triton/compile error for {label} (older GPU detected), skipping this emotion")
                    ...
                    if hasattr(clf, 'parameters'):
                        try:
                            ...
                            self.logger.debug(f"Classifier dtype: {clf_param.dtype}, device: {clf_param.device}")
                        except:
                            pass
        ...
        if raw_scores:
            ...
            emotions = {}
            for i, (label, score) in enumerate(zip(raw_scores.keys(), softmax_scores)):
                emotions[label] = {
                    "score": float(score),
                    "rank": i + 1
                }
            ...
            return {
                ...
                "raw_scores": raw_scores if self.config.get("include_raw_scores", False) else None
            }
    ...
    def get_pipeline_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "models": {
                "whisper_base": self.config.get("whisper_model", "mkrausio/EmoWhisper-AnS-Small-v0.1"),
                "classifier_type": "FullEmbeddingMLP",
                "variant": self.config.get("model_size", "small"),  # Use model_size instead of model_variant
                "emotion_labels": list(EMOTION_LABELS.keys()),
            },
            "config": {
                "segmentation_strategy": self.config.get("segmentation_mode", "fixed_interval"),
