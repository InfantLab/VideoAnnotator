...
class AutomaticPersonLabeler:
    ...
    def label_persons_automatic(self, person_tracks: List[Dict], 
                               frame_annotations: List[Dict]) -> Dict[str, Dict]:
        """
    ...
    def _group_detections_by_person(self, frame_annotations: List[Dict]) -> Dict[str, List[Dict]]:
    ...
    def _apply_size_based_labeling(self, person_detections: Dict[str, List[Dict]]) -> Dict[str, Dict]:
        """
        ...
        if person_heights:
            ...
            if max_height > 0:
                for person_id, height in person_heights.items():
                    ...
                    logger.debug(f"Size-based label: {person_id} -> {label} "
                               f"(height={normalized_height:.3f})")
        
        return labels
    
    def _apply_position_based_labeling(self, person_detections: Dict[str, List[Dict]]) -> Dict[str, Dict]:
        """
        ...
        for person_id, center_bias in person_center_bias.items():
            ...
            if person_id not in labels or labels[person_id]["confidence"] < config["confidence"]:
                ...
                logger.debug(f"Position-based label: {person_id} -> {label} "
                           f"(center_bias={center_bias:.3f})")
        
        return labels
    
    def _apply_temporal_consistency(self, labels: Dict[str, Dict], 
                                  person_detections: Dict[str, List[Dict]]) -> Dict[str, Dict]:
        """
        ...
        for person_id, label_info in labels.items():
            ...
            if detection_count >= config["min_detections"]:
                ...
                detection_confidence = min(1.0, detection_count / (config["min_detections"] * 2))
                ...
                if adjusted_confidence >= config["consistency_threshold"]:
                    ...
                    logger.debug(f"Temporal consistency: {person_id} kept with "
                               f"confidence={adjusted_confidence:.3f} "
                               f"({detection_count} detections)")
                else:
                    logger.debug(f"Temporal consistency: {person_id} filtered out "
    ...
    def analyze_spatial_relationships(self, person_detections: Dict[str, List[Dict]]) -> Dict[str, Any]:
        """
            ...
            for person_id_2 in person_ids[i+1:]:
                ...
                if relative_positions:
                    ...
                    analysis["spatial_patterns"][pair_key] = {
                        ...
                        "interaction_likelihood": "high" if avg_distance < 0.3 else "low"
                    }
        
        return analysis


def infer_person_labels_from_tracks(person_tracks: List[Dict], 
...
def calculate_labeling_confidence(person_detections: Dict[str, List[Dict]]) -> Dict[str, float]:
    """
    ...
    for person_id, detections in person_detections.items():
        ...
        if detections:
            timestamps = [d.get('timestamp', d.get('frame_number', 0)) for d in detections]
