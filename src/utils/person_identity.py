...
class PersonIdentityManager:
    ...
    def __init__(self, video_id: str = None, id_format: str = "semantic", config: Dict[str, Any] = None):
        """
    ...
    def register_track(self, track_id: int) -> str:
        ...
        if track_id not in self.track_to_person_map:
            ...
            self.track_to_person_map[track_id] = person_id
            self.next_person_id += 1
            logger.debug(f"Registered new person: track_id={track_id} -> person_id={person_id}")
    ...
    def set_person_label(self, person_id: str, label: str, confidence: float = 1.0, 
                        method: str = "manual", timestamp: Optional[float] = None):
        """
        ...
        logger.info(f"Set label for {person_id}: {label} (confidence={confidence:.2f}, method={method})")
    ...
    def link_face_to_person(self, face_bbox: List[float], frame_annotations: List[Dict], 
                           iou_threshold: float = 0.5) -> Optional[str]:
        """
        ...
        if best_person_id:
            logger.debug(f"Linked face to person {best_person_id} with IoU={best_iou:.3f}")
    ...
    @classmethod
    def from_person_tracks(cls, person_tracks: List[Dict], video_id: Optional[str] = None) -> 'PersonIdentityManager':
        """
        ...
        logger.info(f"Created PersonIdentityManager from {len(person_tracks)} tracks, "
                   f"found {len(manager.track_to_person_map)} persons")
        
        return manager
    
    def save_person_tracks(self, output_path: str, detections_summary: Optional[Dict] = None):
        """
    ...
    def load_person_tracks(cls, tracks_file: str) -> 'PersonIdentityManager':
        ...
        logger.info(f"Loaded PersonIdentityManager from {tracks_file}: "
...
def normalize_person_label(label: str) -> Optional[str]:
    """
