...
- **Runtime integration, not just shared prose**: VideoAnnotator now bundles Video Annotation Viewer directly — `pip install videoannotator[all]` followed by `videoannotator serve` gets a user both the processing pipeline and the review interface at `/viewer`, same-origin, with no separate deployment step. Video Annotation Viewer remains fully independent and separately installable — this is additive convenience, not a merge of the two codebases. We also added a contract test (`tests/contract/test_viewer_contract.py`) that validates VideoAnnotator's real output against Video Annotation Viewer's actual parsing schema on every CI run, so drift between the two projects is caught automatically rather than discovered by a user. Its first run caught and fixed a real compatibility gap in person-tracking output (identity fields were omitted whenever automatic labeling was disabled), which we take as concrete evidence that these two projects are actively co-maintained, not loosely associated.
